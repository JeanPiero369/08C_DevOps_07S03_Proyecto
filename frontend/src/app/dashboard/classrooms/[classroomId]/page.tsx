...
import { fetchClassroomDetails, fetchGeneralResults, fetchCompetencyResults, fetchClassroomTeachers, fetchClassroomStudents, fetchUserProfile ,fetchStudentListQuiz} from '@/lib/api';
import type { Classroom, Competency, StudentResult, Quiz ,quizStudentDetails} from '@/types/entities';
...
export default function ClassroomDetailsPage() {
  ...
  const [quizStudentDetails, setQuizStudentDetails] = useState<quizStudentDetails[]>([]);
...
const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const data2 = await fetchStudentListQuiz(classroomId,user.id);

      if (data2) {
        setQuizStudentDetails(data2 || []);
      } else {
        setError("No se pudieron cargar los detalles del classroom.");
      }
  ...
  }, [selectedResultsFilter, classroomId, classroomDetails, activeTab, handleFetchResults]);
    ...
    <div className="space-y-6">
      ...
      {!isTeacher && selectedQuizIdForStudentView && currentUser && (
        <StudentViewAttemptDialog
          ...
          studentName={`${currentUser.name} ${currentUser.last_name}`}
