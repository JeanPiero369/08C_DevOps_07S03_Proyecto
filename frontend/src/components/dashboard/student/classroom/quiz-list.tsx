...
import type { Quiz ,quizStudentDetails} from "@/types/entities";
...
export default function StudentQuizList({ quizzes, onStartQuiz, onViewAttempt, currentUserId,classroomId }: StudentQuizListProps) {
  ...
  return (
    <div className="space-y-4">
      {quizzes.map((quiz) => {
        ...
        const showIniciarQuiz = isQuizCurrentlyActive && !quiz.student_has_attemped;
        
        // Condition for "Ver Entrega": ~P (end_time < now -> hasQuizExpired) OR ~Q (studentHasAttemptedAndScoredNonZero)
        const showVerEntrega = quiz.student_has_attemped;

        const showNotSent= !quiz.student_has_attemped && hasQuizExpired;
