...
import type { NewQuizPayload, Question as QuestionType, Competency, AnswerBase, Quiz, GenerateQuizFromDocumentPayload, GenerateQuizFromTextPayload, TypeQuestion ,NewQuizPayloadSchema} from '@/types/entities';
...
export default function CreateQuizDialog({ classroomId, onQuizCreated, children }: CreateQuizDialogProps) {
 ...
 useEffect(() => {
    if (isOpen && !prevIsOpenRef.current) {
      ...
      //console.log(classroomCompetencies)
      if (timerRef.current) {
  ...
  const onSubmit = async (data: CreateQuizFormValues) => {
    const payload: NewQuizPayload = {
      ...
      start_time: formatInTimeZone(data.start_time, 'America/Lima', "yyyy-MM-dd'T'HH:mm:ss.SSS"),
      end_time: formatInTimeZone(data.end_time, 'America/Lima', "yyyy-MM-dd'T'HH:mm:ss.SSS"),
      ...
      const questionsWithoutCompetencies = data.questions.filter(
    q => !q.competences_id || q.competences_id.length === 0
  );

  if (questionsWithoutCompetencies.length > 0) {
    toast({
      title: "Validación Fallida",
      description: "Todas las preguntas deben tener al menos una competencia asociada.",
      variant: "destructive",
    });
    return; // Detiene el envío del formulario
  }
    try {
      const newQuiz = await createQuiz(payload,classroomId);
      toast({
        title: "Quiz Creado",
        description: `El quiz "${payload.title}" ha sido creado exitosamente.`,
      });
    ...
    try {
        ...
        if (source === "pdf") {
            ...
            const payload: GenerateQuizFromDocumentPayload = {
                classroom_id: Number(classroomId),
                num_question: Number(aiData.num_question),
                point_max: Number(aiData.point_max),
