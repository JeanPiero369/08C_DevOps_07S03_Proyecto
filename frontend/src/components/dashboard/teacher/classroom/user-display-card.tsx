...
const getInitials = (name?: string, last_name?: string) => {
  if (!name) return "??";
  return `${name.charAt(0)}${last_name ? last_name.charAt(0) : ''}`.toUpperCase();
};
      ...
      <CardContent className="p-4">
        <div className="flex items-start space-x-4">
          <Avatar className="h-16 w-16 border-2 border-primary/50">
            <AvatarImage 
                src={`https://placehold.co/64x64.png?text=${getInitials(user.name, user.last_name)}`} 
          ...
          <div className="flex-1 space-y-1">
            <div className="flex justify-between items-start">
                <div>
                    <h3 className="text-lg font-semibold text-foreground">{user.name} {user.last_name}</h3>
