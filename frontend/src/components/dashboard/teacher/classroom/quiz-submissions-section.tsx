...
export default function QuizSubmissionsSection({ quiz, onClose }: QuizSubmissionsSectionProps) {
  ...
  const handleViewStudentAttempt = (studentId: string, studentName: string, studentlast_name: string) => {
    setSelectedStudentIdForDetails(studentId);
    setSelectedStudentNameForDetails(`${studentName} ${studentlast_name}`);
              ...
              <TableBody>
                {submissions.map((sub) => (
                  <TableRow key={sub.student_id}>
                    <TableCell className="font-medium">{sub.name} {sub.last_name}</TableCell>
                    <TableCell className="text-right font-semibold text-primary">{sub.points_obtained} / {quiz.total_points || 'N/A'}</TableCell>
                    <TableCell className="text-center">
                      <Button 
                        variant="secondary" 
                        size="sm"
                        onClick={() => handleViewStudentAttempt(sub.id, sub.student_name, sub.student_last_name)}
                      >
                        Ver Detalle
                      </Button>
                    </TableCell>
