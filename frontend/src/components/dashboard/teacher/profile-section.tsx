    ...
    <Card className="shadow-lg w-full max-w-2xl mx-auto">
      <CardHeader className="items-center text-center border-b pb-6">
        <Avatar className="w-24 h-24 mb-4 ring-2 ring-primary ring-offset-2 ring-offset-background">
          <AvatarImage src={`https://placehold.co/100x100.png?text=${getInitials(user.name, user.last_name)}`} alt={`${user.name} ${user.last_name}`} data-ai-hint="profile person" />
          <AvatarFallback>{getInitials(user.name, user.last_name)}</AvatarFallback>
        </Avatar>
        <CardTitle className="text-3xl font-headline">{user.name} {user.last_name}</CardTitle>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-lg font-medium p-2 bg-muted/30 rounded-md min-h-[40px] flex items-center">{user.last_name}</p>
                    )}
                    <FormMessage />
                  </FormItem>
                )}
